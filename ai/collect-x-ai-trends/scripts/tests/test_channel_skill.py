...
SCRIPT = SKILL_DIR / "scripts" / "collect_x_ai_trends.py"
...
def load_module():
    spec = importlib.util.spec_from_file_location("collect_x_ai_trends", SCRIPT)
...
class XChannelSkillTests(unittest.TestCase):
    def test_fixture_run_emits_only_x_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = subprocess.run(
                [sys.executable, str(SCRIPT), "--config", str(config_path), "--fixture-dir", str(FIXTURES),
                 "--output-dir", str(root / "out"), "--run-id", "20260717T000000Z-x"],
            ...
            self.assertIn("raw/x.json", manifest["files"])
    ...
    def test_defaults_cover_coding_agent_topics(self) -> None:
        config = json.loads(CONFIG.read_text(encoding="utf-8"))
        queries = " ".join(config["x"]["topic_queries"]).lower()
        self.assertIn("coding agent", queries)
        self.assertIn("ai coding", queries)
