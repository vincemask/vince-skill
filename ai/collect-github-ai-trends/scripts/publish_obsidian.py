...
OBSIDIAN_CONTENT_CHUNK_BYTES = 3500
...
def split_content_for_obsidian(value: str, limit: int = OBSIDIAN_CONTENT_CHUNK_BYTES) -> list[str]:
    """Keep CLI arguments small and avoid splitting a UTF-8 sequence."""
    if limit <= 0:
    ...
    while remaining:
        ...
        chunk = candidate[:newline + 1] if newline >= 0 else candidate
        if not chunk:
            raise ValueError("无法生成安全的 Obsidian 内容分块")
        chunks.append(chunk)
...
def create_or_verify_note(cli: ObsidianCLI, plan: dict[str, Any], existing: str | None, actions: list[str]) -> None:
    if existing is None:
        chunks = split_content_for_obsidian(plan["note_content"])
        result = cli.run("create", f"path={plan['note_path']}", f"content={chunks[0]}")
        ...
        for chunk in chunks[1:]:
            result = cli.run("append", f"path={plan['note_path']}", f"content={chunk}", "inline")
            if cli.failed(result):
                detail = safe_detail(result.stderr or result.stdout or f"exit {result.returncode}")
                raise PublishError("note", f"追加趋势笔记失败：{detail}")
...
def update_index(cli: ObsidianCLI, plan: dict[str, Any], content: str, actions: list[str]) -> int:
    ...
    if changed:
        chunks = split_content_for_obsidian(expected)
        result = cli.run("create", f"path={plan['index_path']}", f"content={chunks[0]}", "overwrite")
        for chunk in chunks[1:]:
            append = cli.run("append", f"path={plan['index_path']}", f"content={chunk}", "inline")
            if cli.failed(append):
                detail = safe_detail(append.stderr or append.stdout or f"exit {append.returncode}")
                raise PublishError("index", f"追加 index.md 失败：{detail}")
